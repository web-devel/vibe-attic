...
- While the model generates the initial game, the facilitator explains the architecture.
...
The central rhythm is: describe → generate → play → predict → change → test → explain.
...
The baseline has one obstacle type and a fixed speed. Keep the art simple enough to draw with canvas shapes. Use three files:

| File | Role |
| --- | --- |
| `game/index.html` | Page, instructions, score, restart button, canvas |
| `game/style.css` | Page layout, colors, and typography |
| `game/game.js` | Input, game state, physics, obstacles, collisions, and drawing |
...
| 5–12 | Generation runs while the facilitator explains the architecture | Connect the prompt, model, tools, files, and browser |
| 12–18 | Open and play the game; locate its three files | Connect generated code to visible behavior |
...
Plan for two AI requests per participant: build, then one feature. Repair requests are recovery options, not required lesson stages. Use the inference time for explanation, prediction, or preparing a test. Do not make the explanation depend on generation taking exactly seven minutes.
...
## Architecture explanation during generation
...
| Generated files | Store the HTML, CSS, and JavaScript that make the game |
| Browser | Loads those files, runs JavaScript, and displays the game |
...
Show that the child's short request is only part of the context: the harness also loads the prepared project instructions. Those instructions specify the web stack, files, and useful defaults. The child chooses the desired behavior without needing to repeat that setup. Include this in the later presentation so the result is not presented as coming from the short prompt alone.
...
flowchart LR
    ...
    Harness -->|Read or change| Files[HTML, CSS, JavaScript files]
    Files -->|Contents and tool results| Harness
    Files -->|Loaded by| Browser[Browser runs the game]
```

Show a real file write or edit in the agent interface. Explain that the harness can return errors and file contents to the model, but the model only receives feedback actually supplied to it. Do not imply that it automatically sees the browser, checks game feel, or knows a change worked.
...
Once the files exist, the browser can run this game without the model or harness. Close the coding agent during rehearsal and show that the game still works. With the planned local setup, both generation and gameplay can happen on the laptop; verify this before claiming the workshop works offline.
...
1. Find `jumpSpeed` in `game/game.js`; note the current value.
...
Prepare a verified baseline, a manually adjusted version, and examples of each offered feature. Save the actual typed prompt, a copy of the agent instructions used, and any repair steps with each result. Generated outputs need not match byte for byte.

If inference fails, explain that the group is using a prepared AI-generated version, then continue the manual experiment and inspection. Preserve the participant's current files and stop pending agent edits before restoring a save point. See [save-point conventions](../checkpoints/README.md).
